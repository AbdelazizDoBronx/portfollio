...
const Navbar = () => {
    ...
    const [menuState, setMenuState] = useState(false) // Using boolean state for menu
    ...
    <>
        ...
        <div className='flex gap-7'>
            ...
            <button onClick={() => setMenuState(true)} className='md:hidden'>
                <Image src={assets.menu_black} alt='menuIcon' className='w-5'/>
            </button>
    ...
    {/* Menu for phone */}
    <div className={`md:hidden bg-rose-50 w-64 h-screen fixed ${menuState ? 'right-0' : '-right-64'} bottom-0 top-0 z-50 px-10 py-20 transition-all`}>
        <div>
            <Image onClick={() => setMenuState(false)} src={assets.close_black} alt='close' className='w-5 absolute right-8 top-8 cursor-pointer' />    
        </div>  
